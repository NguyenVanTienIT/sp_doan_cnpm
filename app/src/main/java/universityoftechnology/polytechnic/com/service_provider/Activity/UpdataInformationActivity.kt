...
class UpdataInformationActivity : AppCompatActivity() {
    ...
    fun uploadImage(name: String) {
        var retrofit: Retrofit = ApiClient.getRetrofitClient(this@UpdataInformationActivity);
        var apiService: APIService = retrofit.create(APIService::class.java)
        apiService.uploadFileToServer("application/x-www-form-urlencoded", name, "bg")
    ...
    fun uploadStringUsingRetrofit(image: String) {
        var retrofit: Retrofit = ApiClient.getRetrofitClient(this@UpdataInformationActivity);
        var apiService: APIService = retrofit.create(APIService::class.java)
        var requestFile: ResquestStringFile = ResquestStringFile()
        requestFile.file = image
        //requestFile.file = "abc"

        apiService.uploadImage("application/x-www-form-urlencoded", requestFile, "bg")
            .enqueue(object : Callback<ResponseFile> {
                override fun onResponse(call: Call<ResponseFile>, response: retrofit2.Response<ResponseFile>) {
                    Log.d("ket_qua", response.body()!!.success.toString() + "===" + response.body()!!.data)
                }
    ...
    fun uploadFileUsingRetrofit(file: File) {
        var retrofit: Retrofit = ApiClient.getRetrofitClient(this@UpdataInformationActivity);
        var apiService: APIService = retrofit.create(APIService::class.java)
        var request: RequestFile = RequestFile()
        request.file = file

        var formData: MultipartFormDataBody = MultipartFormDataBody()
        formData.addFilePart("file", file)

        var requestBody: RequestBody = RequestBody.create(MediaType.get("multipart/form-data"), file)
        var body: MultipartBody.Part = MultipartBody.Part.createFormData("file", file.name, requestBody);
        apiService.uploadFile("multipart/form-data; boundary=--------------------------", body, "bg")
            .enqueue(object : Callback<ResponseFile> {
                override fun onResponse(call: Call<ResponseFile>, response: retrofit2.Response<ResponseFile>) {
                    // Log.d("ket_qua", response.body()!!.success.toString() + "===" + response.body()!!.data)
                }
    ...
    fun initData() {
        ...
        /* if (informationUser.background != ""){
             Log.d("ket_qua", getString(R.string.server_image)+"/bg/"+informationUser.background)
             Picasso.get().load(getString(R.string.server_image)+"/bg/"+informationUser.background).into(imageAvatar)
         }*/
        Picasso.get().load(getString(R.string.bg_default)).into(imageAvatar)
    }
            ...
            override fun onClick(v: View?) {
                if (ContextCompat.checkSelfPermission(this@UpdataInformationActivity, Manifest.permission.CAMERA)
                    != PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(
                        this@UpdataInformationActivity,
    ...
    fun uploadImageToServer() {
        ...
        var url: String = Server + "/provider/image/upload?type=bg"
        var requestQueue: RequestQueue = Volley.newRequestQueue(this)
        var stringRequest: StringRequest = object : StringRequest(
            Request.Method.POST, url,
            Response.Listener { s ->
    ...
    fun getStringImage(bmp: Bitmap): String {
        ...
        Log.d("ket_qua", baos.toString())

        var encodedImage = Base64.encodeToString(imageBytes, Base64.DEFAULT);

        /*  Base64Image.instance.encode(bmp) { base64 ->
              base64?.let {
                  // success
                  Log.d("ket_qua_bitmap", base64)
              }
          }*/
        return encodedImage;
    }

    fun updateImageInLocal(fileName: String) {
        var gson: Gson = Gson()
        var json = sharedpreference!!.getString("Information_User", null)
        ...
        informationUser.address = oldInformation.address
        informationUser.email = oldInformation.email
        informationUser.tel = oldInformation.tel
        informationUser.name = oldInformation.tel
        informationUser.background = fileName
    ...
    fun updateInlocal() {
        ...
        informationUser.background = oldInformation.background
