...
class YeuCau_Fragment : Fragment() {
    ...
    fun checkShip(id: String): Boolean {
        if (listRequestShip != null) {
            for (e in listRequestShip!!) {
    ...
    fun checkBook(id: String): Boolean {
        if (listRequestBook != null) {
            for (e in listRequestBook!!) {
                if (e._id == id) return true
            }
        }
        return false
